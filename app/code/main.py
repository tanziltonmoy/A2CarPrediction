...
scaler_path = "model/A1scaler.pkl"
...
app.layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col(html.Div([
            ...
            html.Ul([
                html.Li("Please select one of the two models**(Model_A1 is a Random Forest Model, Model_A2 is a Normal Linear Regression Model"),
        ...
        dbc.Col(dbc.Card([
            ...
            html.Br(),
                ...
                dbc.Row([
                    ...
                    dbc.Tooltip("Petrol or Diesel", target="fuel_dropdown"),
                ]),
                dbc.Row([
                    dbc.Col(dbc.Label("Type of Transmission:"), width=6),
                    dbc.Col(dcc.Dropdown(['Manual', 'Automatic'], id='transmission_dropdown'), width=6),
                    dbc.Tooltip("Manual or Automatic", target="transmission_dropdown"),
                ]),

                 dbc.Row([
                    dbc.Col(dbc.Label("Select Model"), width=6),
                    dbc.Col(dcc.Dropdown(['Model_A1', 'Model_A2'], id="model_dropdown"), width=6),
                    dbc.Tooltip("A1: Random Forest, A2:  Linear Regression", target="model_dropdown"),
                ]),
...
def Prediction(year, km_driven, engine_size, max_power, fuel, transmission, model_dropdown, submit):
    ...
    if model_dropdown == 'Model_A1':
        model = pickle.load(open("model/A1.model", 'rb'))
        prediction = model.predict(input_feature)[0]
    elif model_dropdown == 'Model_A2':

        model2 = pickle.load(open("model/a2model.pkl", 'rb'))
